...
import rpcConfig from 'public/chain_info.mainnet.json';
...
export async function getPoolRatio(pool_id: string): Promise<number> {
    ...
    // Extracting the rpc endpoint from the rpcConfig object
    const rpc: HttpEndpoint = { url: rpcConfig.rpc, headers: {} }; // You can update headers if they are available in rpcConfig
